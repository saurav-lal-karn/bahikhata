...
export const transactionCategoryService = {
    createCategory: async (data: { name: string; type: TransactionType; family_id: string; parent_id?: string }) => {
        const response = await apiClient.post("/transaction-categories", data);
        return response.data.data;
    },
    getCategories: async (familyId: string, includeSystem: boolean = true, type?: string) => {
    ...
    getCategoryById: async (id: string) => {
        const response = await apiClient.get(`/transaction-categories/details/${id}`);
        return response.data.data as TransactionCategory;
    },
    updateCategory: async (id: string, data: Partial<TransactionCategory>) => {
        const response = await apiClient.put(`/transaction-categories/${id}`, data);
        return response.data.data;
    },
