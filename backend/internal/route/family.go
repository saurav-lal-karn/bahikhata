...
func RegisterFamilyRoutes(app *config.Application, rg *gin.RouterGroup) {
	...
	rg.GET("", familyController.List)
	rg.POST("", familyController.Create)
	rg.GET("/:id", familyController.GetByID)
	rg.PUT("/:id", familyController.Update)
	rg.DELETE("/:id", familyController.Delete)
	rg.GET("/:id/stats", familyController.GetStats)
}
